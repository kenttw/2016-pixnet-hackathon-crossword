...
## 資料集
參賽者將獲得完整的**原始本文**與**範例題目**兩個資料集作為訓練資料。在這裡我們先公布範例，於參賽者報名成功後將開放完整資料的下載連結。

* **原始本文集**：來自痞客邦站內分類為流行、旅遊的熱門文章，同時也是本次比賽題目的出處。但我們也要提醒大家，**問題的答案未必都與原始文章如出一轍，當答案不存在於原始文章的時候，我們希望參賽者選出最接近的答案**。就像上面的例子一樣。
* **範例題目**：我們從原始本文集之中整理出來的題目集合，範例與資料格式請看[這裡](https://www.pixnet.net/)。事實上，參賽者也可以不看原始本文集，單純透過解析範例題目，找出問題與答案之中的規則來回答問題。
